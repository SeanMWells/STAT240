...
#### Due Wednesday, November 10, 11:59 PM CT
...
What are the dimensions of `prob1`?    
  -Prob1 is converted into a tibble of 250 rows by 1 column in the first step. It eventually is summarized into a table of 1 row and 2 columns.  
  
What does each row represent?    
  -Each row represents the amount of elements that are nested in each countries' list. 

What does the value of `n` in row 10 of the tibble represent?
  -'n' in row 10 of the tibble represents the number of elements present in the tenth country's list. 

#### B

Are there any countries with different numbers of elements in their list? Briefly justify your response.

  -No, each country should have the same number of elements (23) based on the fact that both the minimum and maximum values of these lengths were 23. 
...
While all map functions can be applied across all elements of a list, the difference between these functions is the type of vector that will be returned (ex. vectors of characters, integers, etc.). 
...
countries_df = countries %>% 
  {
    tibble(
      common_name = map_chr(., pluck, "name", "common"),
      official_name = map_chr(., pluck, "name", "official"),
      num_captitals = map(., pluck, "capital") %>% 
        map_int(length)
    )
  }
head(countries_df)
...
```{r}
countries_df %>%
  filter((num_captitals == 0) | capitals == "") %>%
  print()

countries_df %>%
  mutate(
    num_captitals = case_when(
      capitals == "" ~ "0L",
      TRUE ~ as.character(num_captitals)),
    capitals = case_when(
      num_captitals == "0" ~ "NA_character_",
      num_captitals == "0L" ~ "NA_character_",
      TRUE ~ capitals)
    )    
```
...
```{r}
countries_df %>%
  select(region, num_languages) %>%
  ggplot(aes(x=num_languages, fill=region)) +
  geom_bar() +
  facet_wrap(~ region) +
  labs(x = "Number of Languages", y = "Count") +
  ggtitle("Number of Languages Spoken",
          subtitle = "by Countries per Region")
```