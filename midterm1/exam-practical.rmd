...
Gunther is the 18th most common speaker in the show. 
...
```{r}
main_characters = head(speaker_count, 6)$speaker
main_characters
```
...
```{r}
main_character_str = paste(main_characters, collapse="(?!.)|")
main = friends %>%
  mutate(
    main = case_when(
      str_detect(speaker, main_character_str) ~ speaker,
      TRUE ~ "Other"
    )
  )
main %>%
...
```{r}
speakers %>%
  filter(str_detect(speaker, main_character_str)) %>%
  count(speaker, season) %>%
  ggplot(aes(x=speaker, y=n, fill=speaker)) +
  geom_col() +
  facet_wrap(~ season) +
  scale_x_discrete(label = NULL) +
  labs(x="Character", y="Number of Utterances")
```
...
?i removes the need for case-sensitivity for string detection. Meaning all variations of a character's name (e.g. joey, JOEY, etc.) will match the regular expression of simply "joey".
...
```{r}
prob5 %>%
  pivot_longer(cols=`ross.`:`joey.`, names_to="name", values_to="n") %>%
